import unittest
from ...imbroglio.helpers.parser import Parser
...
# Everything test.
test5_inf = "((5 + min(max(component.x.entity, 9), 11)) / 5)^2"
test5_ans = 9

class CoreParserTest(unittest.TestCase):
  def setUp(self):
    self.tester = 0
    self.component = {"x" : {self.tester : 10}}
    self.parser = Parser(component=self.component, entity=self.tester)
  ...
  def test_can_access_component_value(self):
    ...
    values correctly.
    """
    answer = self.parser("component.x.entity")
